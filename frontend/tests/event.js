...
Quand(
  "j'essaie de remplacer l'annotation du passage {int} par :",
  (block_number, markdown) => {
    let element = cy.get(
      `.lectern>.row:nth-child(${
        block_number + 1
      })>.scholium>.content>.formatted-text`
    );
...
Quand("j'essaie de remplacer le contenu de la glose par :", (markdown) => {
  ...
  cy.get("textarea")
    .type("{selectAll} " + markdown.replaceAll(/[{}]/g, (x) => `{${x}}`))
    .blur();
});
...
Quand("je survole le texte :", (text) => {
  cy.contains('p[title="Highlight in document"]', text.trim()).trigger(
    "mouseover"
  );
});
...
Quand(
  "je découpe la glose en passages numérotés et que je me focalise sur la glose",
  () => {
    cy.click_on_contextual_menu_item(
      ".runningHead .scholium",
...
Quand(
  "je remplace le contenu de la glose par ce qui suit et que je me focalise sur la glose :",
  (markdown) => {
    cy.click_on_text("content", "<TEXT>");
    cy.get("textarea")
      .type("{selectAll} " + markdown.replaceAll(/[{}]/g, (x) => `{${x}}`))
...
Quand("je supprime le lien entre le document principal et la référence", () => {
  cy.get(".reference-dropdown").click();
  cy.click_on_text("Delete reference").click();
});

Quand(
  "j'essaie de créer une glose en gardant {string} comme éditeur",
  (userName) => {
