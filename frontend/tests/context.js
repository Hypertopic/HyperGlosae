...
Soit(
  "un document dont je ne suis pas l'auteur affiché comme document principal",
  () => {
    cy.sign_in("bill", "/");
    cy.create_document_from_scratch();
    cy.set_random_name();
...
Soit(
  "le document contenant l'image 2019_10-13_16_UKR_R_A affiché comme document principal",
  () => {
...
Soit("{string} le document principal", (title) => {
  const uris = {
    ...
    "Treignes, le 8 septembre 2012 (Christophe Lejeune)":
      "/6b56ee657c870dfacd34e9ae4e0643dd",
...
Soit(
  "un document reconnaissable dont je suis l'auteur affiché comme glose",
  () => {
    cy.sign_in("alice", "/");
...
Soit(
  "Un document avec une reference dont je ne suis pas l'auteur affiché comme glose",
  () => {
    cy.sign_in("bill", "/4e1a31e14b032f2fa9e161ee9b009122");
  }
);
...
Soit(
  "une glose dont je suis l'auteur faisant référence uniquement au premier passage",
  () => {
    cy.sign_in("alice");
    cy.get(".create-document").click();
    Step(
      this,
      "j'essaie de remplacer le contenu de la glose par :",
...
Soit(
  "un document dont je suis l'auteur affiché comme glose et dont le type est {string}",
  (name) => {
...
Soit(
  "un document reconnaissable dont je suis l'auteur affiché comme glose et dont le type est {string}",
  (name) => {
    cy.sign_in("alice", "/4e1a31e14b032f2fa9e161ee9b123456");
    cy.create_glose((random = true));
    cy.get(".typeIcon").click();
    cy.contains(".list-group-item", name).click();
    cy.sign_out();
  }
...
Soit("je réutilise ma glose reconnaissable", () => {
...
Soit(
  "un autre document, en plusieurs passages, affiché comme glose et dont je suis l'auteur",
  () => {
    cy.sign_in("alice");
    cy.get(".create-document").click();
    cy.click_on_contextual_menu_item(
      ".runningHead .scholium",
      "Break into numbered passages"
    );
    cy.get(".lectern .row:not(.runningHead) .scholium").should(
...
Soit(
  "un document dont je suis l'auteur affiché comme glose et contenant :",
  (text) => {
    cy.sign_in("alice", "/");
    cy.create_document_from_scratch();
...
Soit("la glose a une référence nomée {string}", (title) => {
  cy.create_empty_reference();
});

Soit(
  "ayant parmi les éditeurs {string} et {string}",
  (userName1, userName2) => {
...
Soit(
  "un document sans champ {string} affiché comme document principal",
  (field) => {
    cy.sign_in("alice", "/");
    cy.create_document_from_scratch();
    cy.url().then((url) => {
      const id = url.split("#")[1];
      cy.request(`/api/${id}`).then(({ body: doc }) => {
        delete doc[field];
        cy.request("PUT", `/api/${id}`, doc).then(() => {
          cy.visit(`/${id}`);
        });
      });
    });
